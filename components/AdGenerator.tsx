    ...
    <div className="min-h-screen bg-[#020202] text-white pt-32 pb-20">
      <div className="max-w-3xl mx-auto px-6">
        ...
        <AnimatePresence mode="wait">
          ...
          {/* Question 7: Custom Colors with React Colorful */}
          {currentQuestion === 7 && (
            <motion.div
              key="q7"
              initial={{ opacity: 0, x: 20 }}
            ...
            >
              <div className="mb-8">
                ...
                <p className="text-gray-400">Select 2-4 colors for your ad design.</p>
              </div>
              ...
              {formData.customColors.length > 0 && (
                <div className="mb-6 p-4 bg-white/5 rounded-xl border border-white/10">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-sm text-gray-400 font-medium">Selected Colors ({formData.customColors.length}/4):</span>
                    {formData.customColors.map((color, index) => (
                      <div key={index} className="flex items-center gap-2 group">
                        <div
                          className="w-10 h-10 rounded-lg border-2 border-white/20 shadow-lg cursor-pointer hover:scale-110 transition-transform"
                        ...
                        <button
                          ...
                          className="text-red-400 hover:text-red-300 opacity-0 group-hover:opacity-100 transition-opacity text-lg font-bold"
                        >
                          ✕
                        </button>
                      </div>
              ...
              <div className="flex flex-col items-center gap-6">
                <div className="relative">
                  {/* Add custom CSS for react-colorful */}
                  <style>{`
                    .react-colorful {
                ...
                <div className="w-full max-w-md">
                  <div className="flex items-center gap-4 p-4 bg-white/5 rounded-xl border border-white/10">
                    ...
                    />
                    <div className="flex-1">
                      <p className="text-sm text-gray-400 mb-1">Current Color</p>
                      <p className="text-xl font-mono font-bold">{currentColor.toUpperCase()}</p>
                    </div>
                    <button
                      onClick={() => {
                        if (formData.customColors.length < 4 && !formData.customColors.includes(currentColor)) {
                          setFormData(prev => ({
                            ...prev,
                            customColors: [...prev.customColors, currentColor]
                          }));
                        }
                      }}
                      disabled={formData.customColors.length >= 4 || formData.customColors.includes(currentColor)}
                  ...
                  <p className="text-sm text-gray-500 mt-3 text-center">
                    {formData.customColors.length === 0 && 'Select at least 1 color to continue'}
                    {formData.customColors.length > 0 && formData.customColors.length < 4 && `Add ${4 - formData.customColors.length} more color${4 - formData.customColors.length > 1 ? 's' : ''} (optional)`}
                    {formData.customColors.length === 4 && 'Maximum colors selected'}
                  </p>
                </div>
              </div>
            </motion.div>
