...
- Aspect ratio: ${aspectRatio}`;
    ...
    // Generate 1 image using Imagen 4 Ultra
    console.log('Step 3: Generating image with Imagen 4 Ultra via REST API...');
    
    const imagenResponse = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/imagen-4.0-ultra-generate-001:generateImages`,
      {
        method: 'POST',
        headers: {
    ...
    if (!imagenResponse.ok) {
      ...
      throw new Error(`Imagen API returned ${imagenResponse.status}: ${errorData}`);
    }

    const imagenData = await imagenResponse.json();
    console.log('Imagen response structure:', JSON.stringify(imagenData, null, 2).substring(0, 500));
